...
__m128i operator^(__m128i lhs, __m128i rhs) { return _mm_xor_si128(lhs, rhs); }
...
// 	__m128i tmp = _mm_and_si128(mask, (bits ^ _mm_slli_epi64(bits,
// delta))); 	return bits ^ tmp ^ _mm_srli_epi64(tmp, delta);
// }
// __m256i delta_swap(__m256i bits, __m256i mask, int delta) {
// 	__m256i tmp = _mm256_and_si256(mask, (bits ^
// _mm256_slli_epi64(bits, delta))); 	return bits ^ tmp ^
// _mm256_srli_epi64(tmp, delta);
// }

// ARM FIX
__m128i delta_swap(__m128i bits, __m128i mask, int delta) {
  __m128i shift = _mm_cvtsi32_si128(delta);
  __m128i tmp = _mm_and_si128(mask, (bits ^ _mm_sll_epi64(bits, shift)));
  return bits ^ tmp ^ _mm_srl_epi64(tmp, shift);
}

__m256i delta_swap(__m256i bits, __m256i mask, int delta) {
  __m128i shift = _mm_cvtsi32_si128(delta);
  __m256i tmp = _mm256_and_si256(mask, (bits ^ _mm256_sll_epi64(bits, shift)));
  return bits ^ tmp ^ _mm256_srl_epi64(tmp, shift);
}
...
void init() {
  ...
  for(int i = 0; i < 8; ++i) {
    ...
    rotr8_shuffle_table_256[i] = _mm256_set_epi8(
        (7 + i) % 8 + 24, (6 + i) % 8 + 24, (5 + i) % 8 + 24, (4 + i) % 8 + 24,
        (3 + i) % 8 + 24, (2 + i) % 8 + 24, (1 + i) % 8 + 24, (0 + i) % 8 + 24,
        (7 + i) % 8 + 16, (6 + i) % 8 + 16, (5 + i) % 8 + 16, (4 + i) % 8 + 16,
        (3 + i) % 8 + 16, (2 + i) % 8 + 16, (1 + i) % 8 + 16, (0 + i) % 8 + 16,
        (7 + i) % 8 + 8, (6 + i) % 8 + 8, (5 + i) % 8 + 8, (4 + i) % 8 + 8,
        (3 + i) % 8 + 8, (2 + i) % 8 + 8, (1 + i) % 8 + 8, (0 + i) % 8 + 8,
        (7 + i) % 8, (6 + i) % 8, (5 + i) % 8, (4 + i) % 8, (3 + i) % 8,
        (2 + i) % 8, (1 + i) % 8, (0 + i) % 8);
...
double_board flipVertical(double_board dbd) {
  return double_board(
      _mm256_shuffle_epi8(dbd, flip_vertical_shuffle_table_256));
}
...
uint64_t flipVertical(uint64_t bits) { return _byteswap_uint64(bits); }
...
board mirrorHorizontal(board bd) {
  ...
  bd = _mm_or_si128(_mm_and_si128(_mm_srli_epi64((__m128i)bd, 1), mask1),
                    _mm_slli_epi64(_mm_and_si128(bd, mask1), 1));
  bd = _mm_or_si128(_mm_and_si128(_mm_srli_epi64((__m128i)bd, 2), mask2),
                    _mm_slli_epi64(_mm_and_si128(bd, mask2), 2));
  bd = _mm_or_si128(_mm_and_si128(_mm_srli_epi64((__m128i)bd, 4), mask3),
...
board flipDiagA8H1(board bd) {
  ...
  __m128i mask3 = _mm_set1_epi64x(UINT64_C(0xf0f0f0f000000000));
...
board rotate180(board bd) { return mirrorHorizontal(flipVertical(bd)); }
...
board rotate90clockwise(board bd) { return flipVertical(flipDiagA8H1(bd)); }
...
board rotate90antiClockwise(board bd) { return flipVertical(flipDiagA1H8(bd)); }
...
__m128i rotr(__m128i bits, int index) {
  board bd(bits);
  return board(_rotr64(bd.player(), index), _rotr64(bd.opponent(), index));
}
...
double_board pseudoRotate45clockwise(double_board dbd) {
  ...
  __m256i mask2 = _mm256_set1_epi8(0x33);
  __m256i mask3 = _mm256_set1_epi8(0x0f);
  __m256i data = __m256i(dbd) ^
                 _mm256_and_si256(mask1, (__m256i(dbd) ^ rotr8_epi64(dbd, 1)));
...
board pseudoRotate45clockwise(board bd) {
  ...
  __m128i mask3 = _mm_set1_epi8(0x0f);
  __m128i data =
      __m128i(bd) ^ _mm_and_si128(mask1, (__m128i(bd) ^ rotr8_epi64(bd, 1)));
...
board definites_horizontal_top(board bd) {
  return board(
      _mm_or_si128(tails(bd), mirrorHorizontal(tails(mirrorHorizontal(bd)))));
}
board definites_horizontal(board bd) {
  return board(
      _mm_or_si128(definites_horizontal_top(bd),
                   flipVertical(definites_horizontal_top(flipVertical(bd)))));
}

board definites(board bd) {
  return board(
      _mm_or_si128(definites_horizontal(bd),
...
int stone_sum(board bd) { return (int)_popcnt64(stones(bd)); }

int bit_to_pos(uint64_t bit) { return (int)_tzcnt_u64(bit); }
...
board toBase3_4x16(board bd) {
  __m128i hi = _mm_and_si128(_mm_srli_epi16((__m128i)bd, 4),
                             _mm_set1_epi8(0x0F)); // SIMDe support
  __m128i lo = _mm_and_si128(bd, _mm_set1_epi8(0x0F));
  __m128i b = _mm_unpacklo_epi8(hi, lo);
  __m128i w = _mm_unpackhi_epi8(hi, lo);
  __m128i table1 = _mm_setr_epi8(0, 1, 3, 4, 9, 10, 12, 13, 27, 28, 30, 31, 36,
                                 37, 39, 40); // base2 -> base3 table
  __m128i table2 = _mm_setr_epi8(0, 2, 6, 8, 18, 20, 24, 26, 54, 56, 60, 62, 72,
                                 74, 78, 80); // base2 -> base3 table * 2
...
std::string toBase81(board bd) {
  ...
  __m128i table1 =
      _mm_setr_epi8(0, 1, 3, 4, 9, 10, 12, 13, 32, 33, 35, 36, 41, 42, 44,
                    45); // 32 * a[3] + 9 * a[2] + 3 * a[1] + a[0]
  __m128i table2 = _mm_setr_epi8(33, 35, 39, 41, 51, 53, 57, 59, 97, 99, 103,
                                 105, 115, 117, 121, 123); // table1 * 2 + 33
  __m128i b_base3 = _mm_shuffle_epi8(table1, b);
  __m128i w_base3 = _mm_shuffle_epi8(table2, w);
...
__m128i div9_epu8(__m128i x) {
  __m128i hi =
      _mm_and_si128(_mm_maddubs_epi16(x, _mm_set1_epi16((short)0x1d00)),
                    _mm_set1_epi16((short)0xFF00));
  __m128i lo =
      _mm_srli_epi16(_mm_maddubs_epi16(x, _mm_set1_epi16((short)0x001d)), 8);
