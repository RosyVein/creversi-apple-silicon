...
#define _blsr_u64(x) (x & (x - 1))
#define _blsi_u64(x) (x & -x)
#define _popcnt64 __builtin_popcountll
#define _bswap64 __builtin_bswap64
#define _rotr64(x, n) __builtin_rotateleft64(x, 64 - n)
#define _tzcnt_u64(x) __builtin_ctzll(x)

#else
#error "Unsupported architecture for SIMD operations!"
#endif

#ifdef _MSC_VER
inline __m256i operator^(__m256i s1, __m256i s2) {
  return _mm256_xor_si256(s1, s2);
}
inline __m128i operator^(__m128i s1, __m128i s2) {
...
inline __m128i operator~(__m128i s1) {
  return _mm_xor_si128(s1, _mm_set1_epi64x(-1));
}
inline __m128i &operator|=(__m128i &s1, __m128i s2) {
  return s1 = _mm_or_si128(s1, s2);
}
#endif

#ifdef __GNUC__
inline unsigned char _bittest64(const long long *a, long long b) {
  return (*a & (1LL << b)) != 0 ? 1 : 0;
}
inline unsigned char _bittestandset64(long long *a, long long b) {
  const long long mask = 1LL << b;
  unsigned char val = (*a & mask) != 0 ? 1 : 0;
  *a |= mask;
  return val;
}
inline unsigned char _bittestandreset64(long long *a, long long b) {
  const long long mask = 1LL << b;
  unsigned char val = (*a & mask) != 0 ? 1 : 0;
  *a &= ~mask;
  return val;
...
struct half_board {
  ...
  bool get(const size_t index) const {
    return _bittest64((const long long *)&data, index);
  }
  bool set(const size_t index) {
    return _bittestandset64((long long *)&data, index);
  }
  bool reset(const size_t index) {
    return _bittestandreset64((long long *)&data, index);
  }
...
struct board {
  ...
  const half_board opponent() const { return _mm_extract_epi64(data, 1); }
