...
inline u64_4 operator>>(const u64_4 lhs, const size_t n) {
  __m128i shift = _mm_cvtsi32_si128((int)n);
  return _mm256_srl_epi64(lhs.data, shift);
}
...
inline u64_4 operator<<(const u64_4 lhs, const size_t n) {
  ...
  __m128i shift = _mm_cvtsi32_si128((int)n);
  return _mm256_sll_epi64(lhs.data, shift);
}
...
inline u64_4 nonzero(const u64_4 lhs) {
  ...
  __m256i cmp = _mm256_cmpeq_epi64(lhs.data, zero);
  return _mm256_add_epi64(cmp, u64_4(1).data);
}
...
board move(const board &bd, int i, int j) { return move(bd, i * 8 + j); }
